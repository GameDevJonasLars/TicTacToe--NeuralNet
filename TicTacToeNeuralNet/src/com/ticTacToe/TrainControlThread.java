	...
	public void run() {
		while(true) {
			...
			if (iKreisPunkte < iKreuzPunkte) {
				iNeural2 = main.getaGeneticAlg().mutate(iNeural1, 5-iKreuzPunkte);
			}
			else if(iKreisPunkte > iKreuzPunkte){
				iNeural1 = main.getaGeneticAlg().mutate(iNeural2, 5-iKreisPunkte);
			}
			else {
				...
				iNeural1 = main.getaGeneticAlg().mutate(iNeural1,5-iKreuzPunkte);
				iNeural2 = main.getaGeneticAlg().mutate(iNeural2,5-iKreisPunkte);
			}
			for(Feld[] felds:main.getfFelder()) {
				for (Feld feld : felds) {
					feld.setBesitzer(0);
